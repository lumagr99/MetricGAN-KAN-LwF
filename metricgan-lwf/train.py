...
class MetricGanBrain(sb.Brain):
    ...
    def lwf_active(self):
        """Returns True if LwF is enabled and has non-zero weight."""
        if not bool(getattr(self.hparams, "lwf_enabled", False)):
            return False
        ...
        return float(getattr(self.hparams, "lambda_lwf", 0.0)) > 0.0

    def current_lambda_lwf(self):
        """Returns per-epoch lambda_lwf with optional linear decay."""
        if not self.lwf_active():
            return 0.0

        total_epochs = max(int(self.hparams.number_of_epochs), 1)
        current_epoch = max(int(getattr(self, "epoch", 1)), 1)
        ...
        base_lambda = float(self.hparams.lambda_lwf)
        decay_factor = float(getattr(self.hparams, "lambda_lwf_decay", 1.0))
    ...
    def maybe_refresh_lwf_snapshots(self, epoch):
        ...
        n_reset = int(getattr(self.hparams, "n_reset", 1))
        if n_reset > 0 and epoch % n_reset == 0:
            self.refresh_lwf_snapshots()
    ...
    def kd_loss(self, student_score, teacher_score, temperature=2.0):
        ...
        return (
            F.kl_div(student_log, teacher_dist, reduction="batchmean")
            * (temperature**2)
        )
    ...
    def compute_objectives(self, predictions, batch, stage, optim_name=""):
        ...
        if stage == sb.Stage.TRAIN:
            ...
            if optim_name == "generator":
                ...
                # LwF (Li & Hoiem, 2017): KD from old discriminator at T=2.
                if (
                    lwf_lambda > 0.0
    ...
    def est_score(self, deg_spec, ref_spec, discriminator=None):
        ...
        return discriminator(combined_spec)
