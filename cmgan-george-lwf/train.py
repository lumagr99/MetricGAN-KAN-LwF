...
class MetricGanBrain(sb.Brain):
    ...
    def lwf_active(self):
        """Returns True when LwF should contribute to the loss."""
        if float(getattr(self.hparams, "lambda_lwf", 0.0)) > 0.0:
            return True

        lambda_start = getattr(self.hparams, "lambda_lwf_start", None)
        lambda_end = getattr(self.hparams, "lambda_lwf_end", None)
        ramp_epochs = int(getattr(self.hparams, "lambda_lwf_ramp_epochs", 0))
        if lambda_start is not None or lambda_end is not None:
            start_val = float(lambda_start) if lambda_start is not None else 0.0
            end_val = float(lambda_end) if lambda_end is not None else start_val
            return max(start_val, end_val) > 0.0

        return ramp_epochs > 0

    def current_lambda_lwf(self, epoch=None):
        """Returns the current LwF weight."""
        ...
        if lambda_start is not None or lambda_end is not None or ramp_epochs > 0:
            start_val = float(lambda_start) if lambda_start is not None else 0.0
            end_val = float(lambda_end) if lambda_end is not None else float(
                getattr(self.hparams, "lambda_lwf", start_val)
            )

            if ramp_epochs > 0:
                current_epoch = max(int(epoch if epoch is not None else getattr(self, "epoch", 1)) - 1, 0)
                progress = min(current_epoch / max(ramp_epochs, 1), 1.0)
            else:
                total_epochs = max(int(getattr(self.hparams, "number_of_epochs", 1)), 1)
                current_epoch = max(int(epoch if epoch is not None else getattr(self, "epoch", 1)), 1)
                progress = 0.0 if total_epochs == 1 else (current_epoch - 1) / (total_epochs - 1)

            return max(0.0, start_val + progress * (end_val - start_val))

        return float(getattr(self.hparams, "lambda_lwf", 0.0))

    def snapshot_module_state(self, module):
        """Stores a frozen copy of module params/buffers for LwF."""
        return {
            "params": OrderedDict(
    ...
    def functional_call_module(self, module, params, buffers, *args, **kwargs):
        """Runs a module with externally provided params and buffers."""
        try:
    ...
    def refresh_lwf_snapshots(self):
        """Refreshes the frozen LwF snapshots."""
        self.old_generator_state = self.snapshot_module_state(self.modules.generator)
        self.old_degenerator_state = self.snapshot_module_state(
            self.modules.degenerator
        )
    ...
    def old_degenerator_forward(self, batch):
        ...
        if self.hparams.degen_mode == "CLEAN":
            source_wavs = clean_wavs
        else:
            source_wavs = noisy_wavs

        source_spec = self.compute_feats_mag_no_power(source_wavs)
        snapshot = self.old_degenerator_state
        ...
        predict_spec = torch.mul(mask, source_spec)
        return self.hparams.resynth(torch.expm1(predict_spec), source_wavs)

    def old_discriminator_forward(self, deg_wav):
        """Runs the discriminator snapshot on the current input."""
    ...
    def compute_objectives(self, predictions, batch, stage, optim_name=""):
        ...
        lwf_lambda = self.current_lambda_lwf()
        lwf_cost = 0.0
        lwf_input_wav = None
        ...
        if stage == sb.Stage.TRAIN:
            ...
            if stage == sb.Stage.TRAIN and lwf_lambda > 0.0:
                if optim_name == "generator" and hasattr(self, "old_generator_state"):
                    with torch.no_grad():
                        old_predict_wav = self.old_generator_forward(batch).detach()
                    lwf_cost = torch.nn.functional.mse_loss(predict_wav, old_predict_wav)
                elif optim_name == "degenerator" and hasattr(
                    self, "old_degenerator_state"
                ):
                    with torch.no_grad():
                        old_predict_wav = self.old_degenerator_forward(batch).detach()
                    lwf_cost = torch.nn.functional.mse_loss(predict_wav, old_predict_wav)
                elif hasattr(self, "old_discriminator_state"):
                    if optim_name == "D_clean":
                        lwf_input_wav = clean_wavs
                    elif optim_name in ("D_enh", "D_denh"):
                        lwf_input_wav = predict_wav
                    elif optim_name == "D_noisy":
                        lwf_input_wav = noisy_wavs

                    if lwf_input_wav is not None:
                        with torch.no_grad():
                            old_est_score = self.old_discriminator_forward(
                                lwf_input_wav
                            ).detach()
                        lwf_cost = torch.nn.functional.mse_loss(est_score, old_est_score)

            if stage == sb.Stage.TRAIN and lwf_lambda > 0.0:
                adv_cost = adv_cost + (lwf_lambda * lwf_cost)
        #print("METRICS AFTER")
        #print(self.metrics)
